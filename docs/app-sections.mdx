...
**Componentes hijos:** `HeroSection`, `ProductShowcase`, `FeaturedReviews`, `OfferShowcase`, `ContactForm`, `AppNavBar`, `AppFooter`
...
#### 5. OfferShowcase (componente)
**Archivo:** `src/components/shared/OfferShowcase.vue`

- Sección de productos en oferta con efecto de impacto visual
- Grid de productos con descuento desde Ecommerce Service (`/api/v1/ecommerce/offers`)
- Badge animado de descuento con efecto `pulse-discount` (escala + glow)
- Countdown de días restantes en cada oferta
- 3D Tilt hover effect (mismo diseño que ProductShowcase)
- Enlace directo al detalle del producto (`ProductPublicDetail`)
- Estados: loading (skeleton cards), error (with retry), empty, success
- Fallback a productos destacados si no hay ofertas activas
- Gestionable desde dashboard: `/app/ecommerce/offers`
